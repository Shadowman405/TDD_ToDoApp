...
class TaskManager {
    ...
    @objc func save() {
        let taskDictioniries = self.tasks.map{$0.dict}
        guard taskDictioniries.count > 0 else {
            try? FileManager.default.removeItem(at: tasksURL)
            return
        }
        
        let plistData = try? PropertyListSerialization.data(fromPropertyList: taskDictioniries, format: .xml, options: PropertyListSerialization.WriteOptions(0))
